...
#
# Method:
#   - Installs rtl8812au-dkms from AUR via yay (auto-installs yay if missing)
#   - Uses DKMS so the driver rebuilds automatically across kernel updates
#
# Usage:
...
# Note: intentionally not forcing a full system upgrade in an installer
...
log "Unloading any currently-loaded RTL88xxAU modules (best effort)..."
...
log "Installing rtl8812au-dkms from AUR (DKMS, rolling-safe)..."
yay -S --needed --noconfirm rtl8812au-dkms

log "Attempting to load module (88XXau / 8812au)..."
sudo modprobe 88XXau 2>/dev/null || sudo modprobe 8812au 2>/dev/null || {
  ...
  die "Failed to load module (tried 88XXau and 8812au)."
}
